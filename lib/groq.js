...
// Note: dangerouslyAllowBrowser is required because this project currently calls AI directly from client components.
const groq = new Groq({
...
export async function generateCourseLayout(prompt) {
    if (!apiKey || apiKey === "dummy_key_for_build") {
        // If running on client and key is missing, throw clear error
...
export async function generateQuiz(topic, difficulty, numberOfQuestions = 10) {
    ...
    try {
        ...
        // Clean up potentially markdown wrapped content just in case
    ...
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "You are an educational content generator. This is for educational purposes only. Output strict JSON only."
                },
                {
                    role: "user",
                    content: prompt
